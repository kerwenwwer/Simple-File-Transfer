/**
 * Simple File Transfer Server For 2019 Introduction to Computer Network Lab3.
 * Author: Kerwin Tsai 
 * 2019/10
 * **/
...
int main(int argc, char **argv) {
	int svr_fd;                   /* socket file descriptor, returu by `socket()` */
	...
	if (svr_fd < 0) {
  	...
  	/* detect shell sub command */
	if(argv[1] == '\0'){
		printf("usage: server <Port> \nTo bind the socket to port, please give a port number first.\n");
		exit(1);
	}
...
void file_listing_handler(int sockfd) {
    ...
    while ((pDirent = readdir(pDir)) != NULL) {
        ...
        write_bytes = write(sockfd, &sendStart, 1);
        ...
        write_bytes = write(sockfd, buf, strlen(buf));
        ...
        char sendEnd = 'f';
        write_bytes = write(sockfd, &sendEnd, 1);
  ...
  closedir(pDir); 
    char end = 'e';
    int end_of_list_bytes = write(sockfd, &end, 1);
    if(!end_of_list_bytes){
        perror("Reading socket ERROR");
        exit(1);    
    }
}
...
void file_sending_handler(int sockfd, char filename[]) {
    ...
    if (fp) {
		...
		if(write_bytes < 0) {
			perror("Server write file size Failed");
			exit(1);
		}  
